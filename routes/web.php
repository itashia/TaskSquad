...
Route::group(['prefix' => 'admin', 'middleware' => 'auth'], static function ($router){
    ...
    $router->get('/groups/create', \App\Livewire\Support\Groups\Create::class)->name('groups.create')->middleware('can:groups_index');;

    // Roles

    $router->get('/roles', \App\Livewire\Support\Roles\Index::class)->name('roles.index')->middleware('can:roles_index');;
    $router->get('/roles/create', \App\Livewire\Support\Roles\Create::class)->name('roles.create')->middleware('can:roles_index');;
    ...
    $router->get('/permissions/create', \App\Livewire\Support\Permissions\Create::class)->name('permissions.create')->middleware('can:permissions_index');;

    // Tasks

    $router->get('/tasks', \App\Livewire\Support\Tasks\Index::class)->name('tasks.index')->middleware('can:tasks_index');
    $router->get('/tasks/create', \App\Livewire\Support\Tasks\Create::class)->name('tasks.create')->middleware('can:tasks_index');;

    // Projects

    $router->get('/projects', \App\Livewire\Support\Projects\Index::class)->name('projects.index')->middleware('can:projects_index');;
    $router->get('/projects/create', \App\Livewire\Support\Projects\Create::class)->name('projects.create')->middleware('can:projects_index');;
